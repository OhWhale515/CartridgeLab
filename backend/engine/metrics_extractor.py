...
def extract_metrics(strat, starting_cash: float, final_value: float, price_data=None) -> dict:
    ...
    raw_trades = _extract_raw_trade_log(strat)
    execution_data = analyze_execution(order_ledger, raw_trades)
    order_lifecycle = execution_data.get("order_lifecycle", [])
...
def _extract_raw_trade_log(strat) -> list:
    """Return analyzer-captured closed trades without execution enrichment."""
    try:
